...
export default function KnowledgeHubPage() {
  ...
  const getLevelColor = () => {
    switch (devLevel) {
      ...
      case "vetted": return "bg-blue-500"
  ...
  const getLevelBadge = () => {
    switch (devLevel) {
      case "none": 
        return <Badge variant="outline" className="text-gray-500 border-gray-300">Not Vetted</Badge>
      case "vetted": 
      ...
      case "platinum": 
        return <Badge className="bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 hover:from-indigo-600 hover:via-purple-600 hover:to-pink-600">Platinum Expert</Badge>
      default: 
          ...
          <div className="space-y-6">
            <div className="space-y-2">
              <div className="flex justify-between text-sm">
                <span>Current Level: <span className="font-medium">{devLevel.charAt(0).toUpperCase() + devLevel.slice(1)}</span></span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="flex items-center gap-3 p-3 rounded-lg border bg-card">
                ...
                <div>
                  <div className="text-sm font-medium">{completedQuizzes.length} / {quizzes.length}</div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 pt-2">
              <div className={`p-4 rounded-lg border ${devLevel === "vetted" || devLevel === "pro" || devLevel === "gold" || devLevel === "platinum" ? "border-blue-200 bg-blue-50" : "bg-gray-50 border-gray-200"}`}>
                <div className="flex justify-between items-start mb-2">
                  <div className={`p-2 rounded-full ${devLevel === "vetted" || devLevel === "pro" || devLevel === "gold" || devLevel === "platinum" ? "bg-blue-100" : "bg-gray-100"}`}>
                    <Shield className={`h-5 w-5 ${devLevel === "vetted" || devLevel === "pro" || devLevel === "gold" || devLevel === "platinum" ? "text-blue-600" : "text-gray-400"}`} />
                  </div>
                  {(devLevel === "vetted" || devLevel === "pro" || devLevel === "gold" || devLevel === "platinum") && (
                    <CheckCircle className="h-5 w-5 text-blue-600" />
                  )}
                </div>
                <h3 className={`font-medium ${devLevel === "vetted" || devLevel === "pro" || devLevel === "gold" || devLevel === "platinum" ? "text-blue-700" : "text-gray-500"}`}>Vetted</h3>
                <p className="text-xs mt-1">Pass screening quiz</p>
              </div>
              
              <div className={`p-4 rounded-lg border ${devLevel === "pro" || devLevel === "gold" || devLevel === "platinum" ? "border-purple-200 bg-purple-50" : "bg-gray-50 border-gray-200"}`}>
                <div className="flex justify-between items-start mb-2">
                  <div className={`p-2 rounded-full ${devLevel === "pro" || devLevel === "gold" || devLevel === "platinum" ? "bg-purple-100" : "bg-gray-100"}`}>
                    <Award className={`h-5 w-5 ${devLevel === "pro" || devLevel === "gold" || devLevel === "platinum" ? "text-purple-600" : "text-gray-400"}`} />
                  </div>
                  {(devLevel === "pro" || devLevel === "gold" || devLevel === "platinum") && (
                    <CheckCircle className="h-5 w-5 text-purple-600" />
                  )}
                </div>
                <h3 className={`font-medium ${devLevel === "pro" || devLevel === "gold" || devLevel === "platinum" ? "text-purple-700" : "text-gray-500"}`}>Pro</h3>
                <p className="text-xs mt-1">5+ tasks, 4.0+ rating</p>
              </div>
              
              <div className={`p-4 rounded-lg border ${devLevel === "gold" || devLevel === "platinum" ? "border-amber-200 bg-amber-50" : "bg-gray-50 border-gray-200"}`}>
                <div className="flex justify-between items-start mb-2">
                  <div className={`p-2 rounded-full ${devLevel === "gold" || devLevel === "platinum" ? "bg-amber-100" : "bg-gray-100"}`}>
                    <Trophy className={`h-5 w-5 ${devLevel === "gold" || devLevel === "platinum" ? "text-amber-600" : "text-gray-400"}`} />
                  </div>
                  {(devLevel === "gold" || devLevel === "platinum") && (
                    <CheckCircle className="h-5 w-5 text-amber-600" />
                  )}
                </div>
                <h3 className={`font-medium ${devLevel === "gold" || devLevel === "platinum" ? "text-amber-700" : "text-gray-500"}`}>Gold</h3>
                <p className="text-xs mt-1">10+ tasks, 4.5+ rating</p>
              </div>
              
              <div className={`p-4 rounded-lg border ${devLevel === "platinum" ? "border-pink-200 bg-pink-50" : "bg-gray-50 border-gray-200"}`}>
                <div className="flex justify-between items-start mb-2">
                  <div className={`p-2 rounded-full ${devLevel === "platinum" ? "bg-pink-100" : "bg-gray-100"}`}>
                    <GraduationCap className={`h-5 w-5 ${devLevel === "platinum" ? "text-pink-600" : "text-gray-400"}`} />
                  </div>
                  {devLevel === "platinum" && (
                    <CheckCircle className="h-5 w-5 text-pink-600" />
                  )}
                </div>
                <h3 className={`font-medium ${devLevel === "platinum" ? "text-pink-700" : "text-gray-500"}`}>Platinum</h3>
          ...
          {quizzes.filter(quiz => !quiz.completed).length === 0 ? (
            <Card>
              <CardContent className="pt-6 text-center">
                ...
                <p className="text-muted-foreground">You've completed all available quizzes. Check back later for new assessments.</p>
              </CardContent>
            </Card>
          ) : (
                ...
                <Card key={quiz.id} className="overflow-hidden">
                  <CardHeader className="pb-2">
                    <div className="flex justify-between items-start">
                      ...
                      {quiz.required && (
                        <Badge variant="outline" className="text-amber-500 border-amber-200 bg-amber-50">Required</Badge>
                      )}
                    </div>
                  ...
                  <CardContent className="pb-2">
                    <div className="flex items-center gap-4 text-sm">
                      ...
                      <div>
                        ...
                        {quiz.level === "platinum" && <Badge className="bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500">Platinum</Badge>}
                      </div>
                    </div>
                  </CardContent>
                  <CardFooter>
          ...
          {completedQuizzes.length === 0 ? (
            <Card>
              <CardContent className="pt-6 text-center">
                ...
                <p className="text-muted-foreground">You haven't completed any quizzes yet. Start with the Developer Screening Quiz.</p>
              </CardContent>
            </Card>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <CardContent className="pb-2">
                    <div className="flex items-center gap-4 text-sm">
                      ...
                      <div>
                        ...
                        {quiz.level === "pro" && <Badge className="bg-purple-500">Pro</Badge>}
                        {quiz.level === "gold" && <Badge className="bg-amber-500">Gold</Badge>}
                        {quiz.level === "platinum" && <Badge className="bg-gradient-to-r from-indigo-500 via-purple-500 to\
