...
const formSchema = z.object({
  ...
  monthlyBudget: z.string().refine((val) => !isNaN(Number(val)) && Number(val) >= 0, {
...
export function VibeCoderProfileForm({ signupData, onUpdate, onBack, onComplete }: VibeCoderProfileFormProps) {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      ...
      const { error: vibeCoderError } = await supabase.from("vibe_coder_profiles").insert({
        ...
        monthly_budget: Number(values.monthlyBudget),
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Input placeholder="Product Manager building the next big thing" {...field} />
                </FormControl>
